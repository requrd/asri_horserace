...
for w_id in id_list:
    ...
    start_race = int(soup.find('section', class_='raceTable').find(
        'a').get('href').split('?')[-1].split('&')[1].split('=')[-1])
    num_race = start_race
    for r in racelist:
    ...
    for no in range(start_race, num_race):
        ...
        # 地方競馬取得
        race_id_nar = str(year) + str(month).zfill(2) + \
            str(day).zfill(2) + str(p_num).zfill(2) + str(course_code)
        date_nar = str(year) + '/' + str(month).zfill(2) + \
            '/' + str(day).zfill(2)
        params_nar = {
            'k_raceDate': date_nar,
            'k_raceNo': p_num,
        ...
        print(title_nar)
        pastBody = soup_nar.find(
            'section', class_='cardTable').find_all('tbody')

        # レース情報
        year = soup.title.string.split("　")[2].split("年")[0]
        month = soup.title.string.split("　")[2].split("年")[
            1].split("月")[0].zfill(2)
        day = soup.title.string.split("　")[2].split(
            "年")[1].split("月")[1].split("日")[0].zfill(2)
        ymd = year + month + day
        ...
        distance = this_info.find("td", width="68%").text.split("　")[
            2][1:].replace('m', '')
        ...
        bd = session.query(BangumiData).filter(
            BangumiData.racekey == race_id).first()
        if bd is None:
            bd = BangumiData()
            bd_exist_flg = 0
        else:
        ...
        for r in tr:
            if len(r) == 21:
                ...
                hp = session.query(HorsePillar).filter(
                    HorsePillar.racehorsekey == rhkey).first()
                if hp is None:
                    hp = HorsePillar()
                    hp_exist_flg = 0
                else:
                ...
                motherfather = w_horse.find(
                    class_="RMFName").text.replace(
                    '母父',
                    '').replace(
                    '(',
                    '').replace(
                ...
                hp.lineage_login_code = aHorsePastRows1.find(
                    'a', class_='horseName').get('href').split('?')[-1].split('=')[-1]
                hp.rider_license_no = aHorsePastRows1.find(
                    'a', class_='jockeyName').get('href').split('?')[-1].split('=')[-1]
                zenso1to5Order = aHorsePastRows1.find_all(
                    'span', class_="pastRank")
                zenso1to5Raceinfo = aHorsePastRows1.find_all(
                    'div', class_='raceInfo')
                raceName = []
                courseCodeList = []
                raceNumList = []
                ...
                for l in range(len(aHorsePastRows2.find_all('td'))):
                    if l > 2:
                        if aHorsePastRows2.find_all(
                                'td')[l].text is not None and aHorsePastRows2.find_all('td')[l].text != '\n':
                            raceName.append(
                                re.split(
                                    '\n',
                        ...
                        if aHorsePastRows2.find_all('td')[l].a is not None:
                            params_w = aHorsePastRows2.find_all('td')[l].a.get(
                                'href').split('?')[-1].split('&')
                ...
                for l in range(len(aHorsePastRows3.find_all('td'))):
                    if l > 2:
                        if aHorsePastRows3.find_all(
                                'td')[l].text is not None and aHorsePastRows3.find_all('td')[l].text != '\n':
                            if len(
                                re.split(
                                    '\u3000|\n| ',
                ...
                for l in range(len(aHorsePastRows4.find_all('td'))):
                    if l > 1:
                        if aHorsePastRows4.find_all(
                                'td')[l].text is not None and aHorsePastRows4.find_all('td')[l].text != '\n':
                            timeCourner.append(
                                re.split(
                                    '\u3000|\n',
                ...
                for l in range(len(aHorsePastRows5.find_all('td'))):
                    if l > 2:
                        if aHorsePastRows5.find_all(
                                'td')[l].text is not None and aHorsePastRows5.find_all('td')[l].text != '\n':
                            ichakusa.append(
                                re.split(
                                    '\u3000|\n',
                ...
                for k in range(len(zenso1to5Order)):
                    ...
                    ymd_z = datetime.datetime.strptime(
                        zensoNRow1_raceInfo_data[1], '%y.%m.%d').strftime('%Y%m%d')
                    ...
                    num_of_all_horse = int(
                        zensoNRow1_raceInfo_data[3].replace(
                            '頭', ''))
        ...
        for r in tr:
            if len(r) == 29:
                ...
                spd = session.query(SpeedData).filter(
                    SpeedData.racehorsekey == rhkey).first()
                if spd is None:
                    spd = SpeedData()
                    spd_exist_flg = 0
                else:
        ...
        for r in tr:
            ...
            spr = session.query(SpeedRankData).filter(
                SpeedRankData.racehorsekey == rhkey).first()
            if spr is None:
                spr = SpeedRankData()
                spr_exist_flg = 0
            else:
        ...
        for r in tr:
            if len(r) == 29:
                ...
                rd = session.query(RecommendData).filter(
                    RecommendData.racehorsekey == rhkey).first()
                if rd is None:
                    rd = RecommendData()
                    rd_exist_flg = 0
                else:
