    ...
    for no in range(start_race, num_race):
        ...
        for r in tr:
            if len(r) == 29:
                ...
                spd = SpeedData(
                    racehorsekey=race_id +
                    str(w_no).zfill(2),
                    sp_mod=sp_mod,
                    sp_mod_mark=sp_mod_mark,
                    sp_mean=sp_mean,
                    sp_mean_mark=sp_mean_mark,
                    sp_max=sp_max,
                    sp_max_mark=sp_max_mark)
                # 前走以前の指数
                tb = r.find_all("table")
                zenso_no = 1
                for s in tb:
        ...
        for r in tr:
            if len(r) == 29:
                ...
                rd = RecommendData(
                    ...
                    hyoka=hyoka,
                    sp=sp,
                    senko_score=senko_score,
                    sp_credit_mark=sp_credit_mark,
                    sp_credit=sp_credit,
                    sp_mod_mark=sp_mod_mark,
                    sp_mod=sp_mod,
                    sp_max_mark=sp_max_mark,
                    sp_max=sp_max,
                    last_leg_power_mark=last_leg_power_mark,
                    last_leg_power=last_leg_power)
                session.add(rd)
                w_no += 1
