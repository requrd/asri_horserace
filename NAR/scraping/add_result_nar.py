...
for w_id in id_list:
    ...
    for no in range(start_race, num_race):
        ...
        return_infos = soup.find_all(
            "td", class_="dbtbl")[1].find(
            "tr", class_="dbdata")
        if return_infos is not None:
            td = return_infos.find_all("td")

        tickets = soup.find_all(
            "td", class_="dbtbl")[1].find_all(
            'td', colspan='3')
        sell_tickets = []
        for r in tickets:
        ...
        if len(sell_tickets) > 3:
            ...
            for s in td[i + 1].childGenerator():
                if str(s).find('br') < 0:
                    setattr(ri, 'sanrenpuku' + str(w) +
                            '_ret', int(re.sub(r'\D', '', s)))
                    w += 1
        ...
        # 個別成績
        pattern = r'([+-]?[0-9]+\.?[0-9]*)'
        horse_results_list = soup.find(
            "td", class_="dbtbl").find_all(
            'tr', align='center')
        for r in horse_results_list:
            ...
            if (horse_results[11].text).find(':') > 0:
                ...
                sd.time_s = int(horse_results[11].text.split(
                    ':')[1]) + (int(horse_results[11].text.split(':')[2]) / 10)
                sd.chakusa = horse_results[12].text
                if len(re.findall(pattern, horse_results[13].text)) > 0:
                    sd.agari_3f = re.findall(
                        pattern, horse_results[13].text)[0]
